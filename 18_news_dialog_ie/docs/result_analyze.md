...
Mistral INT8 запускается с CPU offload части модулей как вынужденный heterogeneous inference для 8 GB VRAM. Основная часть linear weights остаётся квантизованной в INT8, а CPU-offloaded modules хранятся и вычисляются в FP32. Несмотря на смешанное размещение, benchmark сохраняет precision profile `int8`. Численные throughput/F1 здесь намеренно не приводятся до реального завершённого запуска.
