...
Локальное RAG-приложение на Streamlit. Пользователь загружает Markdown и text-файлы, после чего может задавать вопросы только по их содержимому. Для генерации и embedding используются локальные модели Ollama, для наблюдаемости — Langfuse.
...
Загрузка документов, разбиение на фрагменты, создание индекса Chroma, поиск контекста и вызов моделей Ollama.
...
Локальная конфигурация Ollama и ключи Langfuse. Файл не должен попадать в Git.
